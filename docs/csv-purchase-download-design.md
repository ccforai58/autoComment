...
- 用户买完后在插件内直接下载 CSV。
- 下载请求必须携带购买凭证。
- 文件命名和售卖列表都必须体现 CSV 内 `created_at` 的日期范围，例如 `blogs_basic_20260601_20260701.csv`。
- 导出分两类：250 条 `page_as < 50` 的普通包，50 条 `page_as > 50` 的高 AS 包。
...
```text
blogs_basic_YYYYMMDD_YYYYMMDD.csv
blogs_high_as_YYYYMMDD_YYYYMMDD.csv
```

示例：

```text
storage/csv-batches/blogs_basic_20260601_20260701.csv
storage/csv-batches/blogs_high_as_20260601_20260701.csv
```

两个日期分别来自该 CSV 内数据的 `MIN(blog_run_stats.created_at)` 和 `MAX(blog_run_stats.created_at)`。如果极端情况下同一个日期范围生成了多个文件，为避免文件名冲突，可以追加序号：

```text
blogs_basic_20260601_20260701_0002.csv
```
...
- `basic_low_as`：普通包，`page_as < 50`，每 250 条生成一个 CSV。
- `high_as`：高 AS 包，`page_as > 50`，每 50 条生成一个 CSV。
...
WHERE i.id IS NULL
  ...
  AND CAST(TRIM(s.page_as) AS DECIMAL(10,2)) < 50
...
这里先用 `REGEXP` 排除空字符串、非数字字符串，再用 `CAST(... AS DECIMAL(10,2))` 做数值比较。`page_as = 50` 不进入任一批次。

3. 如果普通包不足 250 条，不生成普通 CSV；如果高 AS 包不足 50 条，不生成高 AS CSV。
4. 如果达到对应阈值：
...
如果希望一次任务处理多个批次，可以循环执行，直到普通包剩余不足 250 且高 AS 包剩余不足 50。
...
```json
{
  ...
  "batches": [
    {
      "batchId": 12,
      "batchType": "basic_low_as",
      "batchTypeText": "AS低于50",
      "batchNo": "BLOGS_BASIC_20260601_20260701",
      "fileName": "blogs_basic_20260601_20260701.csv",
    ...
    },
    {
      "batchId": 11,
      "batchType": "high_as",
      "batchTypeText": "AS高于50",
      "batchNo": "BLOGS_HIGH_AS_20260501_20260531",
      "fileName": "blogs_high_as_20260501_20260531.csv",
...
Content-Disposition: attachment; filename="blogs_basic_20260601_20260701.csv"
```
