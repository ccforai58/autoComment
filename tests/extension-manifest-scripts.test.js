...
test('extension manifests load link assistant helpers before content script', () => {
  ...
  for (const manifest of manifests) {
    ...
    const sessionLogicIndex = scripts.indexOf('lib/manual-assistant-session-logic.js');
    ...
    assert.ok(sessionLogicIndex >= 0, 'manual assistant session helper is listed');
    ...
    assert.ok(sessionLogicIndex < contentIndex, 'manual assistant session helper loads before content.js');
  }
});
