...
function scoreFieldRole(input) {
  ...
  const nearbyText = safeLower(source.nearbyText);
  const haystack = [primaryText, nearbyText].filter(Boolean).join(' ');
  const tagName = safeLower(source.tagName);
  const type = safeLower(source.type);
  const ariaRole = safeLower(source.role);
  ...
  const roles = [
    ...
    { role: 'tagline', patterns: [/tagline/, /tag line/, /slogan/, /short tagline/, /one[-\s]?liner/], boost: 0 },
    { role: 'pricingModel', patterns: [/pricing model/, /pricing/, /price model/, /business model/, /free/, /freemium/], boost: ariaRole === 'combobox' ? 1 : 0 },
    { role: 'title', patterns: [/tool name/, /product name/, /app name/, /ai tool name/, /title/, /site name/, /website name/, /listing name/, /name of/], boost: 0 },
    { role: 'author', patterns: [/author/, /your name/, /name/, /contact person/], boost: 0 },
    { role: 'category', patterns: [/category/, /categories/, /topic/], boost: ariaRole === 'combobox' ? 1 : 0 },
  ...
  for (const role of roles) {
    const primaryMatches = role.patterns.filter((pattern) => pattern.test(primaryText));
    const nearbyMatches = role.patterns.filter((pattern) => pattern.test(nearbyText));
    const matches = role.patterns.filter((pattern) => pattern.test(haystack));
    const rawScore = primaryMatches.length * 35 + nearbyMatches.length * 8 + role.boost * 10;
    const score = rawScore > 0 ? Math.min(95, rawScore + 15) : 0;
    if (score > best.score) {
      best = {
        role: role.role,
        score,
...
function buildManualResourcePayload(input) {
  ...
  return {
    promotionProjectId: source.promotionProjectId || source.promotion_project_id || null,
    targetUrl: safeText(source.targetUrl || source.target_url || source.promotionWebsiteUrl),
